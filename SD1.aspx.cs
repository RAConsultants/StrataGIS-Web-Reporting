...
public partial class SD1: System.Web.UI.Page
{
    ...
    protected void fillLists()
    {
        ...
        dataConn.Open();
        sqlquery = "select Department from dbo.SD1_VEHICLE_VIEW group by Department order by Department";
    ...
    private void buildTotal()
    {
        ...
        sqlquery += " ,STUFF  ((SELECT N', ' + CAST(Replace(Replace(MILCodes, '#' + b.VehicleId + '#', ''), 'v', '') AS VarChar(150))   FROM   dbo.SD1_MART_DAILY_REPORT b  WHERE b.MILCodes <> '' AND b.MILCodes IS NOT NULL  AND   a.VehicleId = b.VehicleId AND   ";
        sqlquery += "  b.LocalDate >=  Convert(date,'" + wdpStart.Date.ToString("yyyy-MM-dd") + "') and   b.LocalDate <= Convert(date,'" + wdpEnd.Date.ToString("yyyy-MM-dd") + "') FOR XML PATH('')/*,TYPE*/ ), 1, 2, '') as MILCodes ";
        sqlquery += " FROM [SD1].[dbo].[SD1_MART_DAILY_REPORT] a ";
        string whereClause = " where LocalDate>=  Convert(date,'" + wdpStart.Date.ToString("yyyy-MM-dd") + "') and   LocalDate <= Convert(date,'" + wdpEnd.Date.ToString("yyyy-MM-dd") + "') ";
    ...
    protected void buildEnterExit()
    {
        ...
        dataConn.Open();
        string sqlquery = "select LocalDate as Date, Department as Dpt, Time, VehicleId as Vehicle, EnterExit";
    ...
    protected void lbDept_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            dataConn.Open();
            string sqlquery = "select VehicleId from dbo.SD1_VEHICLE_VIEW where  " + deptWhere() + " order by VehicleId";
            SqlCommand command = new SqlCommand(sqlquery, dataConn);
            SqlDataReader reader = command.ExecuteReader();
            try
            {
